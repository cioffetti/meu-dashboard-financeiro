...
# --- 2. LISTAS DE ATIVOS (COM CONTROLE DE CASAS DECIMAIS) ---
# Formato: "Nome de Exibição": ("Ticker", Número_de_Casas)
macro_dict = {
    "Dólar": ("USDBRL=X", 3), "Euro": ("EURBRL=X", 3),
    "Ouro": ("GC=F", 2), "Petróleo (WTI)": ("CL=F", 2),
...
# Transforma a lista BR em dicionário definindo 2 casas decimais para todas
...
# Transforma a lista EUA em dicionário definindo 2 casas decimais para todas
...
def renderizar_grid_cards(dicionario_ativos):
    # Extrai apenas os tickers para o motor de busca (ignora o número de casas aqui)
    lista_tickers = [info[0] for info in dicionario_ativos.values()]
    dados_lote = buscar_dados_em_lote(lista_tickers)
                    ...
                    if len(precos) >= 2:
                        ...
                        with cols[j]:
                            with st.container(border=True):
                                # Passa a variável 'casas' específica de cada ativo
