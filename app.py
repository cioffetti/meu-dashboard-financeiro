...
def gerar_relatorio_ia(ticker, dados_fundos=None):
    ...
    try:
        ...
        **VALUATION E FUNDAMENTOS REAIS DO TERMINAL:**
        ...
        Abaixo estão os cálculos matemáticos precisos e as notícias REAIS coletadas:
        {contexto_dados}
...
if os.path.exists(arquivo_csv):
    ...
    with aba_simulador:
        ...
        with st.expander("Defina seus Pesos de Decisão (0 a 100%)", expanded=True):
            ...
            st.caption("O Simulador utiliza as taxas ao vivo para o DCF.")

        df_sim = df.copy()
        df_sim['Taxa_Apli'] = np.where(df_sim['Origem'].str.contains("BRAPI|Fundamentus"), taxa_selic_live, taxa_us10y_live)
        df_sim['Cresc_Lim'] = df_sim['Crescimento_5a_%'].clip(lower=0, upper=15)
        df_sim['Justo_DCF'] = np.where(df_sim['LPA'] > 0, df_sim['LPA'] * (8.5 + 2 * df_sim['Cresc_Lim']) * (4.4 / df_sim['Taxa_Apli']), 0)
    ...
    with aba_fundamentos: st.warning("⚠️ Execute o 'robo_balancos.py' primeiro para gerar o banco de dados.")
    with aba_simulador: st.warning("⚠️ Execute o 'robo_balancos.py' primeiro para gerar o banco de dados.")

# --- ABA DE ANÁLISES (TÉCNICA + IA NO RAG QUADRANTE) ---
with aba_analises:
    ...
    if col3.button("🧠 Gerar Veredito IA", use_container_width=True):
        ...
        if dados_base_carregados:
            ...
            if not linha.empty:
                ...
                linha_fundo = df_fundo[df_fundo['Ticker'].str.contains(ticker_limpo, na=False)]
                if not linha_fundo.empty:
                    dados_envio['Justo_DCF'] = linha_fundo.iloc[0].get('Justo_DCF', 'N/A')
